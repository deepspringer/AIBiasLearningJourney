...
import { updateUserActivity, addUserMessage } from "@/lib/userActivity";
...
`;
    ...
    useEffect(() => {
        if (userId && displayName) {
            updateUserActivity({
                currentPhase,
                currentParagraph: currentPhase === 1 ? currentParagraph : undefined,
            });
        }
    }, [currentPhase, currentParagraph, userId, displayName]);
          ...
          ${ENGAGEMENT_GUIDANCE}`;
            ...
            if (currentPhase === 1 && newCount >= 2) {
                ...
                const paragraphMessages = allMessages.filter((m) => {
                    ...
                    if (m.role === "assistant" && m.content.includes(`Let's discuss paragraph`))
                        return false;
